...
# Function to load optimal ranges from JSON (ONLY loading now)
# def load_optimal_ranges_json(json_path):
#     try:
...
# def initialize():
#     json_file_path = "assets/data/optimal_ranges.json"
#     try:
#         g.optimal_ranges = load_optimal_ranges_json(json_file_path)
#         if g.optimal_ranges is None:
#             raise Exception("Failed to load optimal ranges from JSON.")
#     except Exception as e:
#         print(f"Failed to initialize optimal ranges: {e}")
#         flask_app.logger.error(f"Failed to initialize optimal ranges: {e}")  # Log the error
#         exit()
...
# def soiloptimization():
#     crops = get_crop_data()
#     selected_crop = request.form.get('crop')  # Get selected crop
#     user_data = session.get('input_data')  # Get user input
#     feedback = {}

#     if selected_crop and user_data:
#         optimal_range = optimal_ranges.get(selected_crop.lower()) # convert selected crop to lowercase so it can match the keys in optimal_ranges
#         if optimal_range:
#             for feature, value in user_data.items():
#                 if feature in optimal_range:
#                     min_val, max_val = optimal_range[feature]
#                     if value < min_val:
#                         feedback[feature] = f"Needs improvement (increase value). Optimal range: {min_val}-{max_val}"
#                     elif value > max_val:
#                         feedback[feature] = f"May be harmful (reduce value). Optimal range: {min_val}-{max_val}"
#                     else:
#                         feedback[feature] = f"Good. Optimal range: {min_val}-{max_val}"
#                 else:
...
#     print(feedback)
#     return render_template("soiloptimization.html", crops=crops, feedback=feedback, selected_crop=selected_crop)
...
def soiloptimization():
    ...
    # if request.method == 'POST': # only check for feedback after form submission
    #     if user_data:
    #         print("User Data:", user_data)
    #         for feature, value in user_data.items():
    #             print(f"Feature: {feature}, Value: {value}, Type: {type(value)}")
    #     if selected_crop and user_data:
    #         optimal_range = optimal_ranges.get(selected_crop.lower())
    #         if optimal_range:
    #             for feature, value in user_data.items():
    ...
    #                     min_val, max_val = optimal_range[feature]
    #                     if value < min_val:
    #                         feedback[feature] = f"Needs improvement (increase value). Optimal range: {round(min_val)}-{round(max_val)}"
    #                     elif value > max_val:
    #                         feedback[feature] = f"May be harmful (reduce value). Optimal range: {round(min_val)}-{round(max_val)}"
    #                     else:
    #                         feedback[feature] = f"Good. Optimal range: {round(min_val)}-{round(max_val)}"
    #             else:
    #                 feedback[feature] = "Optimal range data not available for this feature."
    #         else:
    #             feedback["crop"] = f"Optimal range data not available for {selected_crop}."
